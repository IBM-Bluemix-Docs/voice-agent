...
通过参考材料来了解操作标记或状态变量。操作标记用于启动语音代理程序在对话会话期间执行的操作。状态变量用于定义语音代理程序在整个对话期间持续保持的特征（除非另有更改）。
{: shortdesc}

要查看代码示例以及有关如何设置状态和启动操作序列的示例，请参阅[使用 API 对语音代理程序编程](/docs/services/voice-agent?topic=voice-agent-api)。
