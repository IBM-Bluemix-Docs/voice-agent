...
エージェント・サマリーでボイス・エージェントのすべての番号を確認できます。構成のサマリーが展開された後、**「番号の表示 (View Numbers)」**ボタンをクリックすると、すべての番号のリストが新しい**「管理」**ダイアログ・ボックスに表示されます。**「管理」**ダイアログ・ボックスの操作について詳しくは、[複数のページの管理](/docs/services/voice-agent?topic=voice-agent-multi_num)のページを参照してください。
...
* **[ボイス・エージェントのイベント転送を有効にする](/docs/services/voice-agent?topic=voice-agent-event_forwarding):** より自然な会話を発信者に提供するために、{{site.data.keyword.iva_short}} の通話データを収集、分析することができます。 作成した各ボイス・エージェントから、管理している指定の {{site.data.keyword.cloudant_short_notm}} にイベント・レポートを転送できます。
