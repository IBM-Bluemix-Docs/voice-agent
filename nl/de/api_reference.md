...
| `vgwActSetConversationConfig` | Wendet eine Gruppe von Parametern an, mit denen der Sprachagent einen {{site.data.keyword.conversationshort}}-Arbeitsbereich definieren soll. Der {{site.data.keyword.conversationshort}}-Service definiert die Parameter dynamisch auf Grundlage des Anrufs. | <ul><li>`url`: Der `url`-Berechtigungsnachweis für die {{site.data.keyword.conversationshort}}-Service-API.</li><li>`workspaceID`: ID für den {{site.data.keyword.conversationshort}}-Arbeitsbereich</li><li>`username`: Der `username`-Berechtigungsnachweis für den {{site.data.keyword.conversationshort}}-Service (bis zu 64 Zeichen).</li><li>`password`: Der `password`-Berechtigungsnachweis für den {{site.data.keyword.conversationshort}}-Service (bis zu 256 Zeichen).</li></ul> |
...
| Name der Statusvariablen | Erwarteter Wert | Beschreibung |
| -------------- | ----- | ----------- |
| `vgwSessionID`   | Benutzerdefinierter Wert <br/><br/> Standardwert: `Anruf-ID` | Ein angepasster Header mit der Sitzungs-ID, die aus der SIP-Anforderung 'INVITE' extrahiert wurde. Der Wert stellt die globale Sitzungs-ID dar, die in allen Auditprotokollen des Sprachagenten im Zusammenhang mit der Sitzung verwendet wird. |
